...
#include "umicom/mobile_studio/readiness.h"

int main(void)
{
    UmiApplicationReadinessReport report;
    assert(umi_mobile_studio_readiness_report(&report) == UMI_STATUS_OK);
    ...
    assert(umi_mobile_studio_readiness_next_feature() == report.next_feature);
    return 0;
}
